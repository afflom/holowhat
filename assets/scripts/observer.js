// Attribution: https://colxi.info/#!/projects/deep-observer
  ...
  if (OBSERVED_WS.has(value)) {
    ...
    let ProxyObject = new Proxy(_target, {
      set: function (target, property, value) {
        ...
        if (
          config.ignoreSameValueReassign &&
          action === "update" &&
          oldValue === value
        )
          return true;
        ...
        if (
          isObjLiteralOrArray(value) &&
          (!config.depth || CURRENT_DEPTH < config.depth)
        ) {
          // if value to SET is an Object or Array, create a new
          // proxy, with updated keypath
          let newObserver = createObserver(
            value,
            callback,
            keyPath + "." + property,
            config,
          ...
          constructionCallbacks = constructionCallbacks.concat(
            newObserver.callbacks
          );
